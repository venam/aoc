...
my %scanner_distances;
$scanner_distances{0} = [0,0,0];
my %done;
$done{0} = 1;
while (1) {
		...
		TRANS: for my $k2 (keys %{$pairs_beacons{$k1}}) {
			...
			for my $trans (@xform) {
				...
				if (
					...
					$scanner_distances{$from} = clone(\@moves);
...
#print Dumper(\%scanner_distances);
# try out all combinations of scanner_distancs and keep max
my $maxi = -999;
my $comb = combinations([values(%scanner_distances)], 2);
while (my $pos = $comb->next) {
	my $nval = abs($pos->[0]->[0] - $pos->[1]->[0]) + abs($pos->[0]->[1] - $pos->[1]->[1]) + abs($pos->[0]->[2] - $pos->[1]->[2]);
	$maxi = $maxi > $nval ? $maxi : $nval;
}

print "maximum distance: $maxi\n";
