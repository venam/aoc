...
BINGO: for my $num (@numbers) {
	...
	for my $card (@cards) {
		...
		if (exists $card->{values}->{$num}) {
			...
			if ($win_row == 5) {
				...
				$last_num = $num;
				$win_card = $card;
				$win_card_pos = $card_pos-1;
				push @to_be_removed, $win_card_pos;
				next;
			}
	...
	#last BINGO if (scalar(@cards) == 1);
	if ($last_num != -1) {
		$last_last_num = $last_num;
		$last_num = -1;
		print "card: $win_card_pos won this turn\n";
		$last_win_card = $win_card;
		#print Dumper($win_card);

		# Remove indexes from end of the array first
		for ( sort { $b <=> $a } @to_be_removed ) {
			splice @cards, $_, 1;
		}
...
print "LAST NUM: $last_last_num!!\n";
my $non_marked_sum = 0;
print Dumper($last_win_card);
for my $c (keys %{$last_win_card->{values}}) {
	unless ($last_win_card->{values}->{$c}->{marked}) {
		$non_marked_sum += $c;
	}
}
print "NON MARKED SUM: $non_marked_sum!!\n";
print "MULTIPLIED: ". ($non_marked_sum*$last_last_num)."\n";
